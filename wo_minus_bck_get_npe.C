...
void get_npe_wo_minus_bck(TString list)
{	
	...
	TH2F* htimeAmpSelected = new TH2F("htimeAmpSelected", "a(t) for Cnt",1024,0,1024,4000,0,4000);
	...
	TH1F* hAmpPed = new TH1F("hAmpPed", "Counter amplitude", 2000,-100,1900);
        TH1F* hAmpSigInt = new TH1F("hAmpSigInt", "Counter integral", 500,450000,550000);
        TH1F* hAmpPedInt = new TH1F("hAmpPedInt", "Counter integral", 500,450000,550000);
	//TH1F* hAmpSigInt = new TH1F("hAmpSigInt", "Counter integral", 500,-5000,15000);
        //TH1F* hAmpPedInt = new TH1F("hAmpPedInt", "Counter integral", 500,-5000,15000);
	
	TH1F* hAmpSig2 = new TH1F("hAmpSig2", "Counter amplitude", 2000,-100,1900);
	...
	//Double_t X1 = 60.0, X2 = 76.0; 
	//Double_t Y1 = 27.0, Y2 = 42.0; //mm Geometrical borders

	Double_t X1 = 61.0, X2 = 68.0; 
	Double_t Y1 = 21.0, Y2 = 37.0; //mm Geometrical borders
	
	//Double_t X1 = 63.0, X2 = 69.0; 
	//Double_t Y1 = 26.0, Y2 = 36.0; //mm Geometrical borders
	
	Int_t t1L=200, t1R=10000, t2L=200, t2R=10000; //trig amps borders

	Int_t CEnL=500, CEnR=10000; //calorimeter energy borders

	//sipm signal in range form 635 to 715
	...
	//T=45
	//float A1phe = 42.322; //U56V      //22.0807
	float A1phe = 47.1637; //U57V     //29.8345 
	//float A1phe = 60.465; //U58V	    //37.6456
        //**********************************************
        ...
        //for(Int_t i=0; i<1000; i++)
	{
		...
		DATA="ch1.amp:ch1.ti >> amp";
        	exbeamdata->Draw(DATA, CONDITION,"",1,i);
		
		if( exbeamdata->Draw(DATA, CONDITION,"",1,i)>0 )
		{
			TF1* fitl = new TF1("fitl","pol0",binSig1-100.,binSig1-1.);
			amp->Fit("fitl","LQ","",binSig1-100.,binSig1-1.);
			...
			TProfile* amp1 = new TProfile("amp1", "a(t) for Cnt", 1024, 0. , 1024);
	                for(int k=1; k<=amp->GetNbinsX(); k++)
        	        {
                	   Double_t binAmp1 = amp->GetBinContent(k);
                   	   amp1->Fill( k, binAmp1-p0 );
                	}


 			TF1* fitP = new TF1("fitP", PulseShape, binSig1-10, binSig2-40, 5);
	                fitP->SetParameters(0,10,540,15,100);
        	        amp1->Fit("fitP", "LQ", "", binSig1-10, binSig2-40);
			
			//float e_res = =exp((tm-x)/tau); 
			//float res = fitP->GetParameter(0) + fitP->GetParameter(1)*1*pow((1+fitP->GetParameter(4)-1)/fitP->GetParameter(4),fitP->GetParameter(4));;
 			//cout<<"\t"<<res<<endl;
			
			//hAmpSig2->Fill(res);
			hAmpSig2->Fill(fitP->GetParameter(1));
	...
	for(int n1=0; n1<sigA.size(); n1++)
        {
	   hnpeAmpSig->Fill((sigA[n1]-pedVal)/A1phe);	
        }
	hnpeAmpSig->Draw();
